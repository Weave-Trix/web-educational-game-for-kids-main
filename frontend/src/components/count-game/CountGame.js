...
export default function CountGame() {
  ...
  useEffect(() => {
    ...
    if (gesture === null || gesture === undefined) {
      setInfoHand("Unable to recognize hand");
      setIsAnswered(false);
      return;
    } else {
      setInfoHand("Analyzing posture, keep your hand steady......");
    }
    ...
    const interval = setInterval(async () => {
      if (count === 0) {
        ...
        if (userAnswer === currentQuestion) {
          ...
          setInfoHand("Congratulations, your answer is correct! Moving on to the next question!");
          setUserAnswers((prevState) => {
            return {
              ...prevState,
          ...
          setIsAnswerCorrect(false);
          setInfoHand("Wrong answer! You are being redirected to another question...");
          setUserAnswers((prevState) => {
            return {
              ...prevState,
              [questions[currentQuestionIndex].number]: {
            ...
            <Card>
              <Card.Body>
                {isGameOver ? (
                  <Alert variant="info">
                    <Alert.Heading className="text-center">
                    Game over! You are being redirected to the panel...
                    </Alert.Heading>
                  </Alert>
                ) : (
                  <div>
