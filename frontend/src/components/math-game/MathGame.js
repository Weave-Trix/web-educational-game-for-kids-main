...
export default function MathGame() {
  ...
  useEffect(() => {
    ...
    if (gesture === null || gesture === undefined) {
      setInfoHand("Unable to detect hand!");
      setVariant("warning");
      setIsAnswered(false);
      return;
    } else {
      setInfoHand("Analyzing posture... Keep your hand steady! " + timer);
      setVariant("info");
    }
    ...
    const interval = setInterval(async () => {
      if (count === 0) {
        ...
        if (userAnswer === currentQuestionAnswer) {
          ...
          setInfoHand(
            "Congratulations! Correct answer. You are being redirected to another question..."
          );
          setTimer("");
          setUserAnswers((prevState) => {
            return {
          ...
          setIsAnswerCorrect(false);
          setTimer("");
          setInfoHand("Wrong answer! You are being redirected to the next question...");
          setUserAnswers((prevState) => {
            return {
              ...prevState,
              [currentQuestionIndex]: {
  ...
  useEffect(() => {
    if (currentQuestionIndex === questions.length && userAnswers != "") {
      ...
      if (coldStart) {
        ...
        setIsGameOver(true);
        setInfoHand("Game over! You are being redirected to the home page...");
        setVariant("info");
      }
    } else {
      try {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex === questions.length - 1 && userAnswers == "") {
      setInfoHand("No answers found! You are being redirected to the home page...");
      setVariant("info");
      setTimeout(() => {
        window.location.href = "/dashboard";
      }, 1500);
        ...
        <Row className="d-flex justify-content-left">
          ...
          <Col md={4} className="text-center">
            {isGameOver ? (
              ...
              <div>
                <p>The camera is off. Questions not found.</p>
              </div>
            )}
          </Col>
        </Row>
